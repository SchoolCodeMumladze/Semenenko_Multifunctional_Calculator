...
def f4(nums):
    print(f"Среднее арифметическое чисел равно {sum(nums) / len(str(nums))}")
...
def f8(st_de):
    de = map(str, input("Укажите ненужные символы строки через пробел "))
    st_de_new = ""
    for i in st_de:
        if i not in de:
            st_de_new += i
...
if super_function in ["1", "2"]:
    ...
    nums = map(float, input("Введите числа в одну строку через пробел ").split())
    if super_function == "3":
        f3(nums)
    elif super_function == "4":
        f4(nums)
elif super_function in ["5", "6.1", "6.2", "7", "8", "9", "10", "11", "12"]:
...
Что нового в последних версиях:
    1. Добавлена функция перевода числа по системам счисления (16)
    2. Добавлена функция перевода числа в научную запись (17)
    3. Оптимизирована функция операции с дробями (13)
    4. Исправление багов
    """)
